...
class ToggleWidgetProvider : AppWidgetProvider() {
    ...
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        // Called when widgets should be updated
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == ACTION_TOGGLE) {
            ...
            if (appWidgetId != AppWidgetManager.INVALID_APPWIDGET_ID) {
                ...
                // Update all instances if no specific ID provided
                val mgr = AppWidgetManager.getInstance(context)
    ...
    private fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
        ...
        // Wire clicks to broadcast back to this provider
        ...
        // Assign the same pending intent to both views so either click toggles
    ...
    private fun ensureDefaultState(context: Context, appWidgetId: Int) {
        ...
        if (!p.contains(stateKey(appWidgetId))) {
            p.edit().putBoolean(stateKey(appWidgetId), false).apply() // default: show button
        }
    }
